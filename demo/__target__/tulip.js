...
export var TulipJS =  __class__ ('TulipJS', [object], {
	...
	get __init__ () {return __get__ (this, function (self, d) {
		...
		self.link = d ['link'];
		self.local = dict ({'GrpSkip': null, 'LineSkip': null, 'GrpSpan': null, 'LineSpan': null});
	});},
...
export var change_to_list = function (tulip) {
	tulip.to_list ();
	regen (tulip_obj);
};
export var change_to_table = function (tulip) {
	tulip.to_table ();
	regen (tulip_obj);
};
export var change_to_row_maj = function (tulip) {
	tulip.to_row_maj ();
	regen (tulip_obj);
};
export var change_to_col_maj = function (tulip) {
	tulip.to_col_maj ();
	regen (tulip_obj);
};
...
export var _html_table_recursion = function (tulip, level, submem) {
	...
	html += str_repeat (indent, level) + (col_major ? ('<button onclick="tulip.change_to_row_maj(tulip.tulip_obj' + submem) + ')">To Row Maj</button>\n' : ('<button onclick="tulip.change_to_col_maj(tulip.tulip_obj' + submem) + ')">To Col Maj</button>\n');
